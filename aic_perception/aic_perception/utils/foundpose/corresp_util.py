...
def establish_correspondences(
    ...
    if feat_matching_type == "cyclic_buddies":
        ...
        if template_knn_indices is None:
            template_knn_indices = {}

    # Establish correspondences for each dominant template separately.
    corresps = []
    for template_counter, template_id_tensor in enumerate(template_ids):
        template_id = int(template_id_tensor.item())
        ...
        if feat_matching_type == "cyclic_buddies":
            ...
            if template_id not in template_knn_indices:
                template_knn_index = knn_util.KNN(k=1, metric="l2")
                template_knn_index.fit(object_repre.feat_vectors[tpl_feat_ids])
                template_knn_indices[template_id] = template_knn_index
            (
                match_query_ids,
                match_obj_ids,
                match_dists,
